...
class ActionPredict(object):
    ...
    def load_images_crop_and_process(self, img_sequences, bbox_sequences,
        ...
        path_to_pose, _ = get_path(save_folder='poses',
                                           dataset=dataset,
                                           save_root_folder='./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/data/features')
        poses_all = []
        set_poses_list = [x for x in os.listdir(path_to_pose) if x.endswith('.pkl')]
        set_poses = {}
        for s in set_poses_list:
            with open(os.path.join(path_to_pose, s), 'rb') as fid:
                try:
                    p = pickle.load(fid)
                except:
                    p = pickle.load(fid, encoding='bytes')
        set_poses[s.split('.pkl')[0].split('_')[-1]] = p
        hub_layer = hub.KerasLayer("https://tfhub.dev/sayakpaul/vit_s16_fe/1", trainable=False)
        with tf.device('/cpu:0'):
            transformer_model = Sequential([hub_layer]) #InputLayer((224, 224, 3)),
            ...
            segmodel_path = "./Ahmed_Elgazwy/deeplabv3_cityscapes_train_2018_02_05.tar.gz"
        ...
        for seq, pid in zip(img_sequences, ped_ids):
            ...
            for imp, b, p in zip(seq, bbox_seq[i], pid):
                ...
                k = img_name + '_' + p[0]
                if Test_iti<5:
                    print('Testing!!!',imp,b,p,set_poses[set_id2][vid_id][k],k)
                    Test_iti+=1
                if k in set_poses[set_id2][vid_id].keys():
                    pose = set_poses[set_id2][vid_id][k].copy()
               # for i in range(len(pose)):
                   # if i%2==0:
                       # pose[i]=b[0]+(b[2]-b[0])*pose[i]
                   # else:
                       # pose[i]=b[1]+(b[3]-b[1])*pose[i]
                keypoints = np.reshape(pose,(18,2))
                ...
                if os.path.exists(img_save_path) and not regen_data:
                    ...
                    elif crop_type == 'local_context_cnn_vision':
                        img_data = cv2.imread(imp)
                        img_data=cv2.cvtColor(img_data, cv2.COLOR_BGR2RGB)
                        #for k in keypoints:
                           # cv2.circle(img_data, (int(k[0]), int(k[1])), 2, (0,255,0), -1)
                        #cv2.rectangle(img_data,(b[0],b[1]),(b[2],b[3]),[0,255,0],2)
                        img=cv2.resize(img_data, (224, 224))
                        x = image.img_to_array(img)
                        x = (x - 127.5) / 127.5
                        x = np.expand_dims(x, axis=0)
                        block4_pool_features = transformer_model.predict(x)
    ...
    def get_data_sequence(self, data_type, data_raw, opts):
        ...
        d['tte'] = []
        print(len(d['box']),len(d['center']),len(d['ped_id']),len(d['crossing']))
        if isinstance(time_to_event, int):
            ...
            overlap = opts['overlap']
             # if data_type == 'train' else 0.0
            olap_res = obs_length if overlap == 0 else int((1 - overlap) * obs_length)
            olap_res = 1 if olap_res < 1 else olap_res
            print(opts['overlap'],obs_length,olap_res,time_to_event[0],time_to_event[1])
            for k in d.keys():
    ...
    def get_context_data(self, model_opts, data, data_type, feature_type):
        ...
        data_gen_params = {'data_type': data_type, 'crop_type': 'none','dataset' :model_opts['dataset'],
        ...
        data_gen_params['save_path'], _ = get_path(save_folder=save_folder_name,
                                                   dataset=dataset, save_root_folder='./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/data/features')
    ...
    def train(self, data_train,
        ...
        path_params = {'save_folder': os.path.join(self.__class__.__name__, model_folder_name),
                       ...
                       'dataset': model_opts['dataset']}
        model_path, _ = get_path(**path_params, file_name='model')
    ...
    def test(self, data_test, model_path=''):
        ...
        with open(os.path.join(model_path, 'configs.yaml'), 'r') as fid:
            ...
            #print(opts['model_opts'])
            # try:
            #     model_opts = pickle.load(fid)
            # except:
            #     model_opts = pickle.load(fid, encoding='bytes')
        load_time=time.time()
        test_model = load_model(os.path.join(model_path, 'model'))
        ...
        np.savetxt("./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/resultsss.csv", np.round(test_results).reshape((3,-1)), delimiter=",")
        print(test_data['data'][1].shape)
        #print(test_data['image'])
        with open('pcpa.csv', 'w', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=' ',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
           # for k in range(5):
            #    print(test_data['data'][1][k][0],np.round(test_results[k][0]))
            for i,j in zip(test_data['image'],range(1881)):
                if (test_data['data'][1][j][0]!=np.round(test_results[j][0])):
                    spamwriter.writerow(str(i))

        print(np.round(test_results).shape)
        ...
        print('----------------------------------------------------------------------------------')
        print(classification_report(test_data['data'][1], np.round(test_results), labels=[0, 1]))
        cm = confusion_matrix(test_data['data'][1], np.round(test_results), labels = [0,1])
        label_font = {'size':'18'} 
        ...
        print('ACC : ',ACC)
        fpr, tpr, _ = metrics.roc_curve(test_data['data'][1],test_results)
        #tpr1=np.array(tpr)
        #print('fpr',fpr,'tpr',tpr)
        #a_file = open("./Ahmed_Elgazwy/file1.csv", "w+")
        
        #np.savetxt(a_file, tpr1)
        #a_file.close()
        #fpr1=np.array(fpr)
        
        #a_file = open("./Ahmed_Elgazwy/file.csv", "w+")
        
        #np.savetxt(a_file, fpr1)
        #a_file.close()
        plt.plot(fpr,tpr,color = '#FF00C9')
        plt.ylabel('True Positive Rate')
        plt.xlabel('False Positive Rate')
        plt.title("ROC for Static model")
        plt.show()
        #fpr, fnr, _ = metrics.det_curve(test_data['data'][1],test_results)
        #display = metrics.DetCurveDisplay(
        #fpr=fpr, fnr=fnr)
        #display.plot(color= '#B600FF')
        #plt.title("DET for Static model")
        #plt.show()
        end=time.time()-start
        ...
        print('acc:{:.2f} auc:{:.2f} f1:{:.2f} precision:{:.2f} recall:{:.2f} time:{:.2f}'.format(acc, auc, f1, precision, recall,end))
...
def attention_3d_block(hidden_states, dense_size=128, modality=''):
    ...
    def get_model(self, data_params):
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
        plot_model(net_model, to_file='./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/model_imgs/MASK_PCPA.png')
        return net_model
...
class PCPA(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
      #  plot_model(net_model, to_file='./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/model_imgs/PCPA.png')
        return net_model
...
class MASK_PCPA_4_2D(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        net_model = Model(inputs=network_inputs,
                          outputs=model_output)
        net_model.summary()
       # plot_model(net_model, to_file='./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/model_imgs/MASK_PCPA_4_2D.png')
        return net_model
...
class DataGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        if self.to_fit:
            ...
            return X, y # may change in training
        else:
            return [X]
...
#         return config  
  ...
  def call(self, x):
    ...
    if(self.d_model%2==0):
        ...
        x = x + self.pos_encoding[tf.newaxis, :length, :-1]
    return x
...
class GlobalSelfAttention(BaseAttention):
  def call(self, x):
    ...
    x = self.layernorm(x)
    return x
class LocalSelfAttention(BaseAttention):  # Non overlapping local self attention
  def call(self, x,win_size):
    batch,seq_len,embid=x.shape
    num_windows = seq_len // win_size
    splits = tf.split(x, num_or_size_splits=win_size, axis=1)
    for i,j in zip(splits,range(len(splits))):
        attn_output = self.mha(
            query=i,
            value=i,
            key=i)
        if j==0:
            ...
            out = tf.concat([out, attn_output], 1)
    x = self.add([x, out])
    x = self.layernorm(x)
    return x
class FeedForward(tf.keras.layers.Layer):
  def __init__(self, d_model, dff, dropout_rate=0.1,name=None):
    super().__init__()
    self._name=name
...
class EncoderLayer(tf.keras.layers.Layer):
  def __init__(self,*, d_model, num_heads, dff, dropout_rate=0.1,name=None):
    super().__init__()
    self._name=name
    self.self_attention = GlobalSelfAttention(name='GSA' + self._name[2:],
        num_heads=num_heads,
        key_dim=d_model,
        dropout=dropout_rate)

    self.ffn = FeedForward(d_model, dff,name='FFN'+self._name[2:])
#   def get_config(self):
#         config= super().get_config().copy()
#         config.update({
...
#         return config  
  def call(self, x):
    ...
    x = self.ffn(x)
    return x
class EncoderLayer_Local(tf.keras.layers.Layer):
  def __init__(self,*, d_model, num_heads, dff, dropout_rate=0.1,name=None):
    super().__init__()
    self._name=name
    self.self_attention = LocalSelfAttention(name='LSA' + self._name[2:],
        num_heads=num_heads,
        key_dim=d_model,
        dropout=dropout_rate)
...
#         return config  
  def call(self, x,win_size):
    x = self.self_attention(x,win_size)
    x = self.ffn(x)
    return x
class Encoder(tf.keras.layers.Layer):
  def __init__(self, *, num_layers=1, num_heads=1,
               dff=2048, dropout_rate=0.1,name=None):
    super().__init__()
    self._name=name
    #self.d_model = d_model
  ...
  def build(self, input_shape):
    ...
    #print(self.d_model)
    self.pos_embedding = PositionalEmbedding(name='pos' + self._name[3:],
         d_model=self.d_model)

    self.enc_layers = [
        EncoderLayer(name='el' + self._name[3:],
            d_model=self.d_model,
                     num_heads=self.num_heads,
                     dff=self.dff,
                     dropout_rate=0.1)
...
class Encoder_Local(tf.keras.layers.Layer):
  def __init__(self, *, num_layers=1, num_heads=1,
               dff=2048, dropout_rate=0.1,name=None):
    super().__init__()
    self._name=name
  ...
  def build(self, input_shape):
    ...
    self.enc_layers = [
        EncoderLayer_Local(name='ell' + self._name[3:],
            d_model=self.d_model,
                     num_heads=self.num_heads,
                     dff=self.dff,
                     dropout_rate=0.1)
...
#         return config  
  def call(self, x,win_size):
    ...
    for i in range(self.num_layers):
      x = self.enc_layers[i](x,win_size)

    return x  # Shape `(batch_size, seq_len, d_model)`.
class T_E_Model(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        x = Encoder(name='enc0_' + data_types[0])(network_inputs[0])
        encoder_outputs.append(x)
        x = Encoder(name='enc1_' + data_types[1] )(network_inputs[1])
        encoder_outputs.append(x)
        x = Encoder(name='enc2_' + data_types[2])(network_inputs[2])
        # current = [x, network_inputs[1]]
        # x = Concatenate(name='concat_early1', axis=2)(current)
        # x = self._rnn(name='enc1_' + data_types[1], r_sequence=return_sequence)(x)
        # current = [x, network_inputs[2]]
        ...
        current = [x, network_inputs[3]]
        x = Concatenate(name='concat_early3', axis=2)(current)
        x = Encoder(name='enc3_' + data_types[3])(x)
        current = [x,network_inputs[4]]
        x = Concatenate(name='concat_early4', axis=2)(current)
        x = Encoder(name='enc4_' + data_types[4])(x)
        x = tf.keras.layers.Dense(384,name = 'concat_dense')(x)
        ...
        model_output = Dense(1, activation='sigmoid',
                             name='output_dense',
                             activity_regularizer=regularizers.l2(0.001))(encodings) #try encodings with attention
       ...
       # plot_model(net_model, to_file='./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/model_imgs/MASK_PCPA_4_2D.png')
        return net_model
class T_E_Model_Local(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        x = Encoder_Local(name='enc0_' + data_types[0])(network_inputs[0],4)
        encoder_outputs.append(x)
        x = Encoder_Local(name='enc1_' + data_types[1] )(network_inputs[1],4)
        encoder_outputs.append(x)
        x = Encoder_Local(name='enc2_' + data_types[2])(network_inputs[2],4)
        # current = [x, network_inputs[1]]
        # x = Concatenate(name='concat_early1', axis=2)(current)
        # x = self._rnn(name='enc1_' + data_types[1], r_sequence=return_sequence)(x)
        # current = [x, network_inputs[2]]
        ...
        current = [x, network_inputs[3]]
        x = Concatenate(name='concat_early3', axis=2)(current)
        x = Encoder_Local(name='enc3_' + data_types[3])(x,4)
        current = [x,network_inputs[4]]
        x = Concatenate(name='concat_early4', axis=2)(current)
        x = Encoder_Local(name='enc4_' + data_types[4])(x,4)
        x = tf.keras.layers.Dense(384,name = 'concat_dense')(x)
       ...
       # plot_model(net_model, to_file='./Ahmed_Elgazwy/Pedestrian_Crossing_Intention_Prediction-main/model_imgs/MASK_PCPA_4_2D.png')
        return net_model
class T_E_Model_vision(ActionPredict):

    """
    Transformer based
    Class with visual features only
    ...
    def get_model(self, data_params):
        ...
        x = Encoder(name='enc1_' + data_types[1] )(network_inputs[1])
        encoder_outputs.append(x)
        x = Encoder(name='enc2_' + data_types[4])(network_inputs[4])
        x = tf.keras.layers.Dense(384,name = 'concat_dense')(x)
...
class T_E_noglobal(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        #x = Encoder(name='enc1_' + data_types[1] )(network_inputs[1])
        #encoder_outputs.append(x)
        x = Encoder(name='enc2_' + data_types[1])(network_inputs[1])
        # current = [x, network_inputs[1]]
        # x = Concatenate(name='concat_early1', axis=2)(current)
        # x = self._rnn(name='enc1_' + data_types[1], r_sequence=return_sequence)(x)
        # current = [x, network_inputs[2]]
        ...
        current = [x, network_inputs[2]]
        x = Concatenate(name='concat_early3', axis=2)(current)
        x = Encoder(name='enc3_' + data_types[2])(x)
        current = [x,network_inputs[3]]
        x = Concatenate(name='concat_early4', axis=2)(current)
        x = Encoder(name='enc4_' + data_types[3])(x)
        x = tf.keras.layers.Dense(384,name = 'concat_dense')(x)
