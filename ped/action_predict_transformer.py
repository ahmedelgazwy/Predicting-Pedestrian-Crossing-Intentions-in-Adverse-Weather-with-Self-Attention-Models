...
class ActionPredict(object):
   ...
   # @profile
    def test(self, data_test, model_path=''):
        ...
        #print(test_data['ped_id'][0:11])
        with open('listfile.txt', 'w') as filehandle:
         for listitem in test_data['ped_id']:
            filehandle.write('%s\n' % listitem)
        start=time.time()
...
class T_E_Model(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        #x1 = Encoder(name='enc3_' + data_types[3])(network_inputs[3])
        current = [x,network_inputs[4]]
        x = Concatenate(name='concat_early4', axis=2)(current)
        x = Encoder(name='enc4_' + data_types[4])(x)
        #x2 = Encoder(name='enc4_' + data_types[4])(network_inputs[4])
        #current=[x,x1,x2]
        #x=Concatenate(name='concat_early4', axis=2)(current)
       # x = FeedForward(d_model=x.shape[-1], dff=2048,name='feedfwrdlayer') (x)
