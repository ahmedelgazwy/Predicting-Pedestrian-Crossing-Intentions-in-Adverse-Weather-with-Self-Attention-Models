...
class ActionPredict(object):
    ...
    def test(self, data_test, model_path=''):
        ...
        #sns.heatmap(cm/np.sum(cf_matrix), annot=True, fmt='.2%', cmap='Blues'
        ...
        #a_file = open("./ghadeer/file_tpr_pcpa.csv", "w+")
        
        #np.savetxt(a_file, tpr1)
        #a_file.close()
        #fpr1=np.array(fpr)
        
        #a_file = open("./ghadeer/file_fpr_pcpa.csv", "w+")
        ...
        #for i in fpr:
         #   print("i", i)
            #a_file = open("./ghadeer/file_fpr_pcpa_DET", "w+")
            #np.savetxt(a_file, i)
            #a_file.close()
        #for j in fnr:
         #   print("j", j)
           # a_file = open("./ghadeer/file_fnr_pcpa_DET", "w+")
            #np.savetxt(a_file, j)
            #a_file.close()

        #fpr1=np.array(fpr)
        #a_file = open("./ghadeer/file_fpr_pcpa_DET.csv", "w+")
        #np.savetxt(a_file, fpr1)
        #a_file.close()
        #fnr1=np.array(fnr)
        
        #a_file = open("./ghadeer/file_fnr_pcpa_DET.csv", "w+")
        
        #np.savetxt(a_file, fnr1)
        #a_file.close()
        display = metrics.DetCurveDisplay(
        fpr=fpr, fnr=fnr)
        display.plot(color= '#B600FF')
        plt.title("DET for C3D model")
        plt.grid()
        plt.show()
...
class T_E_Model(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        x = tf.keras.layers.Dense(512,name = 'concat_dense')(x)
